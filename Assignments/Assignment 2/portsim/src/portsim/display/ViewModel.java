...
public class ViewModel {
    /**
     * {@link Port} model containing ships and quays
     */
     ...
     */
    private final StringProperty shipInfoText = new SimpleStringProperty("No ship selected");
     ...
     */
    private final StringProperty timeText = new SimpleStringProperty("Time: --:--");

    /**
     * Text displayed in the label showing the evaluator statistics
     */
    private final StringProperty evaluatorsText = new SimpleStringProperty("Evaluators Text");
     ...
     *     null), then the event handler should return immediately without taking any further
     *     action.
     *     </li>
     ...
     *     cargoManifestText} property should be set to the {@link BulkCargo#toString()}
     *     representation
     ...
     * @ass2
     */
    public EventHandler<ActionEvent> getShipContentsHandler() {     // todo test this
        return contentsHandler -> {
            ...
            if (ship instanceof BulkCarrier) {
                if (((BulkCarrier) ship).getCargo() == null) {
                    ...
                    cargoManifestText.setValue(((BulkCarrier) ship).getCargo().toString());
                }
            } else if (ship instanceof ContainerShip) {
                if (((ContainerShip) ship).getCargo() == null) {
                    cargoManifestText.setValue("No cargo on board");
                } else {
                    StringBuilder cargoString = new StringBuilder();
                    for (Container container : ((ContainerShip) ship).getCargo()) {
                        cargoString.append(container.toString()).append(",")
                                .append(System.lineSeparator());
                    }

                    cargoManifestText.setValue(cargoString.delete(cargoString.length() - 2,
                            cargoString.length()).toString());      // todo check this
                }
            }
        };
    }
     ...
     *     The contents of each evaluator is as follows with each bullet point indicating a new
     *     line.
     *         <ol>
     *             <li>If the port has a {@link QuayOccupancyEvaluator}:
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>The current number of Quay's occupied should be printed as the
     *                     following format <pre>num Quay(s) currently occupied</pre> where num is
     *                     the number of occupied quays</li>
     *                 </ul>
     *             </li>
     *             <li>If the port has a {@link ShipFlagEvaluator}:
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>For each unique Country flag that the evaluator has seen it should
     *                     print that statistic in the following format
     *                     <pre>country : num</pre> where country is the name of the country flag
     *                     and num is the number of times that flag has been seen</li>
     *                 </ul>
     *             </li>
     *             <li>If the port has a {@link ShipThroughputEvaluator}:
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>The current number of ships that have passed through the port in the
     *                     last hour should be printed as the following format
     *                     <pre>num Ships passed in the last hour</pre> where num is the number of
     *                     ships through the port in the last hour</li>
     *                 </ul>
     *             </li>
     *             <li>If the port has a {@link CargoDecompositionEvaluator}:
     *                 <ul>
     *                     <li>The name of the evaluator</li>
     *                     <li>For each unique cargo type that the evaluator has seen it should
     *                     print that statistic in the following format <pre>cargo : num</pre>
     *                     where cargo is the name of the cargo class and num is the number of
     *                     times that flag has been seen</li>
     *                 </ul>
     *             </li>
     *         </ol>
     *     </li>
     ...
     * @ass2
     */
    public void updateEvaluatorText() {     // todo test
        if (port.getEvaluators().size() == 0) {
            evaluatorsText.setValue("No Evaluators Present");
            return;
        }

        StringBuilder evalTxtBuildr = new StringBuilder();
        for (StatisticsEvaluator eval : port.getEvaluators()) {
            evalTxtBuildr.append(eval.getClass().getSimpleName()).append(System.lineSeparator());
            switch (eval.getClass().getSimpleName()) {
                case "QuayOccupancyEvaluator":
                    evalTxtBuildr.append(((QuayOccupancyEvaluator) eval).getQuaysOccupied());
                    evalTxtBuildr.append(" Quay");
                    if (((QuayOccupancyEvaluator) eval).getQuaysOccupied() != 1) {
                        evalTxtBuildr.append("s");
                    }

                    evalTxtBuildr.append(" currently occupied");
                    evalTxtBuildr.append(System.lineSeparator());

                    break;
                case "ShipFlagEvaluator":
                    for (String flag : ((ShipFlagEvaluator) eval).getFlagDistribution().keySet()) {
                        evalTxtBuildr.append(flag);
                        evalTxtBuildr.append(" : ");
                        evalTxtBuildr.append(((ShipFlagEvaluator) eval).getFlagDistribution()
                                .get(flag));
                        evalTxtBuildr.append(System.lineSeparator());
                    }
                ...
                case "ShipThroughputEvaluator":
                    evalTxtBuildr.append(((ShipThroughputEvaluator) eval).getThroughputPerHour());
                    evalTxtBuildr.append(" Ships passed in the last hour");
                    evalTxtBuildr.append(System.lineSeparator());

                    break;
                case "CargoDecompositionEvaluator":
                    Map<String, Integer> cargoDistribution =
                            ((CargoDecompositionEvaluator) eval).getCargoDistribution();
                    for (String cargoType : cargoDistribution.keySet()) {
                        evalTxtBuildr.append(cargoType);
                        evalTxtBuildr.append(" : ");
                        evalTxtBuildr.append(cargoDistribution.get(cargoType));
                        evalTxtBuildr.append(System.lineSeparator());
                    }
        ...
        evaluatorsText.setValue(evalTxtBuildr.toString());
    }
     ...
     * Where:
     * <ul>
     *     <li>Name is the name of the {@link Port}</li>
     ...
     *     simulation</li>
     *     <li>numQuays is the total number of quays in the {@link Port}</li>
     ...
     *     <li>numCargo is the total amount of stored cargo in the {@link Port}</li>
     *     <li>cargoID is each cargo's ID in the stored cargo list of {@link Port}</li>
     *     <li>numMovements is the number of movements in the queue of movements
     *     in Port</li>
     *     <li>EncodedMovement is the encoded representation of each individual {@link Movement} in
     *     the
     *     aforementioned list</li>
     *     <li>numEvaluators is the number of statistics evaluators in the {@link Port}</li>
     ...
     * @throws IOException if an {@link IOException} occurs when writing to the writer
     * @ass2
     * @see Port#encode()
     */
    public void saveAs(Writer portWriter) throws IOException {  // todo test.
        BufferedWriter bufferedPortWriter = new BufferedWriter(portWriter);

        bufferedPortWriter.write(port.encode());

        bufferedPortWriter.flush();
        bufferedPortWriter.close();
    }
     ...
     *
     * @throws IOException if an {@link IOException} occurs when writing to the file
     * @given
     */
